...
def scrape_standings() -> dict:
    ...
    Strategy that survives site restructures:
      - Find all tables on the page
      - Identify data tables by whether headers contain W, L, Runs For, Runs Against
      - For each row, get team_id from any team-page link (not column position)
      - Read stats by header name lookup (position-agnostic)
      - Assign division from our known roster (never trust the page for this)
    """
    ...
    for tbl_idx, table in enumerate(all_tables):
        ...
        # Locate the columns we need (must have all of these to be a data table)
        ...
        for tr in rows[1:]:
            ...
            # Extract team_id from ANY link in the row pointing to a team page
            team_id = None
            ...
            # If we already have this team with more games played, skip
            # (later tables sometimes have less complete data)
            ...
            teams[team_id] = Team(
                team_id=team_id,
                name=roster_name,   # use clean roster name (page cells have tiebreaker junk)
                captain=roster_capt,
                division=division,
                wins=wins,
                losses=losses,
    ...
    # Ensure all 18 teams exist (blank ones for anyone we somehow missed)
    for tid, (name, capt, div) in ROSTER.items():
        if tid not in teams:
            teams[tid] = Team(team_id=tid, name=name, captain=capt, division=div)

    div_counts = {"East": 0, "Central": 0, "West": 0}
    for t in teams.values():
        if t.games_played > 0:
            div_counts[t.division] += 1
    print(f"[debug] Tables matched: {tables_matched}")
    print(f"[debug] Teams with games played: East={div_counts['East']}, "
          f"Central={div_counts['Central']}, West={div_counts['West']}")
    print(f"[debug] Total teams in dataset: {len(teams)}")
...
def compute_rankings(teams: dict, week: int | None = None) -> list:
    ...
    def opp_winpct(team: Team) -> float:
        ...
        div_avg = (sum(t.win_pct for t in div_teams) / len(div_teams)
                   if div_teams else 0.5)
        other_avg = (sum(t.win_pct for t in others) / len(others)
                     if others else 0.5)
    ...
    for _ in range(15):
        new_rating = {}
        for tid, t in teams.items():
            div_opps = [teams[o] for o in TEAM_ID_BY_DIVISION[t.division]
                        if o != tid]
            opp_avg = (sum(rating[o.team_id] for o in div_opps) / len(div_opps)
                       if div_opps else 0.0)
